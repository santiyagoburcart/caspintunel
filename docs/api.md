...
| messaging | `notifications/` (+ `broadcast/`) · `channels/` (required channels) · `integrations/telegram/` · `bots/stats/` · `bots/backup-test/` · `email/` (SMTP relay settings, write-only password) · `email/test/` |
